...
module "jenkins_integration_ssm_parameters" {
  source      = "./tdr-terraform-modules/ssm_parameter"
  common_tags = local.common_tags
  parameters = [
    { name = "/${local.environment}/jenkins_url", description = "The url for the jenkins server", type = "SecureString", value = "http://${module.jenkins_integration_alb.alb_dns_name}" }
  ]
}
